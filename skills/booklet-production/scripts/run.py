...
from timing import command,traced,now
with command('booklet-production'):
 ...
 def picture(image,base,sendable,kind,observations,sources):
  ...
  sources[str(p)]={'sha256':sha(p),'width':width,'height':height,'embeddedWidth':im.width,'embeddedHeight':im.height,'role':image.get('role'),'shotId':image.get('shotId'),'source':image.get('source'),'schemeId':image.get('schemeId'),'sceneKey':image.get('sceneKey')}
  if image.get('resultPath'):
   receipt_path=(base/image['resultPath']).resolve();receipt=json.loads(receipt_path.read_text(encoding='utf8'))
   if receipt.get('sha256')!=sha(p):raise ValueError('Booklet image differs from its native result')
   if image.get('shotId')!=receipt.get('shotId'):raise ValueError('Booklet shotId differs from native result')
   sources[str(p)]['resultPath']=str(receipt_path);sources[str(p)]['sceneKey']=receipt.get('sceneKey')
   if receipt.get('status')!='accepted':observations.append({'image':p.name,'issue':'Native result has unresolved visual observations'})
  elif image.get('role')=='render':observations.append({'image':p.name,'issue':'Native result provenance not linked; inspect actual generation evidence'})
 ...
 def build(source,out):
  ...
  with sync_playwright() as p:
   ...
   try:
    for mode in ['highres','sendable']:
     ...
     report['outputs'][mode]['timing']={'startedAt':started,'finishedAt':now(),'elapsedMs':round((time.perf_counter()-clock)*1000,3)}
     doc=pymupdf.open(pdf);dpi=[]
     for index,printed in enumerate(doc):
      for info in printed.get_image_info():
       box=pymupdf.Rect(info['bbox']);effective=min(info['width']/(box.width/72),info['height']/(box.height/72)) if box.width and box.height else 0
       dpi.append({'page':index+1,'effectiveDpi':round(effective,1),'pixels':[info['width'],info['height']]})
       if mode=='highres' and effective<150:report['observations'].append({'page':index+1,'issue':'Print image below 150 effective DPI; do not call upscaling restored detail'})
     doc.close();report['outputs'][mode]['imageDpi']=dpi
  ...
  report['defaultDelivery']='booklet-highres.pdf'

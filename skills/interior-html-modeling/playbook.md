...
按 floor、rooms、walls、openings、placements 构建全户型。墙厚和洞口真实生成，2D 与 3D 共用数据。组件通过 catalog 的 componentId 和实际 GLB 数据决定形体，不用随手堆方块冒充下载模型。已有五种风格可用于初次细部造型，但类别内结构须由选型决定：sofa.straight为直排双人，sofa.cloud为三座贵妃位。不能把一件贵妃位仅缩窄充当直排。选到产品后若结构家族不符，回布局componentId再编译；风格只改变该家族细部/CMF，不能强加贵妃位。此后换风格按钮只更新CMF。

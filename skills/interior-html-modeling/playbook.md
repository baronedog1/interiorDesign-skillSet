...
按 floor、rooms、walls、openings、placements 构建全户型。墙厚和洞口真实生成，2D 与 3D 共用数据。组件通过 catalog 的 componentId 和实际 GLB 数据决定形体，不用随手堆方块冒充下载模型。已有五种风格可用于初次造型选型，但此后换风格按钮只更新 CMF。

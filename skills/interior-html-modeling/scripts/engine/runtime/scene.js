...
(function(C,T){'use strict';
 ...
 C.buildScene=function(root,P){
  ...
  for(const r of C.rooms){const floorGroup=C.group(architecture,r.name+'地面');floorGroup.userData.roomId=r.id;floorGroup.userData.surfaceType='floor';const rect=r.polygon.length===4&&r.polygon.every(p=>(p[0]===r.bounds[0]||p[0]===r.bounds[1])&&(p[1]===r.bounds[2]||p[1]===r.bounds[3]));const kind=['kitchen','bathroom'].includes(r.type)?'stone':'wood';if(rect)C.buildFloorRect(floorGroup,r.bounds,kind,r.name+'地面');else polygonMesh(floorGroup,r.polygon,.004,kind==='wood'?C.M.oak:C.M.travertine,r.name+'地面');}
  for(const w of P.walls){const ops=P.openings.filter(o=>o.wallId===w.id).map(o=>({id:o.id,type:o.type==='sliding-door'?'sliding':['passage'].includes(o.type)?'passage':['window','fixed-glazing'].includes(o.type)?'window':'door',c:o.offset+o.width/2,w:o.width,b:o.sill,h:o.height,assembly:window.OPENING_STATES[o.id]}));let g;
  ...
  const roof=C.group(root,'02 完整天花 / 灯槽');C.ceiling=roof;roof.userData.surfaceType="ceiling";roof.visible=false;const ceilingMat=C.M.plaster.clone();ceilingMat.side=T.DoubleSide;polygonMesh(roof,P.floor.outline,P.floor.height,ceilingMat,'完整顶面');
  const groups=new Map();for(const r of C.rooms){const g=C.group(root,r.name);g.userData.roomId=r.id;groups.set(r.id,g);}
  for(const p of P.placements){const o=p.nativeAssetId?C.nativeAssets.create(groups.get(p.roomId),p):C.instantiate(groups.get(p.roomId),p,window.COMPONENT_CATALOG);if(!p.nativeAssetId&&p.materialOverrides)o.traverse(x=>{if(!x.material)return;const arr=Array.isArray(x.material)?x.material:[x.material];const next=arr.map(m=>{const patch=p.materialOverrides[m.name];if(!patch)return m;const a=m.clone();a.userData.cmfOverride=true;if(patch.color)a.color.set(patch.color);if(Number.isFinite(patch.roughness))a.roughness=patch.roughness;if(Number.isFinite(patch.metalness))a.metalness=patch.metalness;return a;});x.material=Array.isArray(x.material)?next:next[0];});}

...
#### 先问清：门后是什么，靠什么分隔
- source：原图 + 现场照片 + 已有需求；识别每间房用途与边界，不从家具外观猜空间
  - playbook/requirements-interview.md：只追问影响布局的未知项
- known：连接与开合资料明确？；两端空间、门型、透明性、开合、外部目标
  - ../interior-html-modeling/playbook/space-connections.md：空间身份、连接、开合、视野交接及默认值唯一合同
- ask：分轮询问或标注设计假设；未知不写成确认；null不自动等于户外；可继续基础结构草稿，不设问卷填满门槛
- write：写入唯一布局 JSON；rooms / openings.connects / openConnections；门型、开合、玻璃与exteriorView；窗有视觉连接，但不作为通行门
  - ../interior-html-modeling/scripts/engine/schemas/layout.schema.json：字段与单位
  - ../interior-html-modeling/scripts/engine/python/validate.py：视觉相邻与通行图分离
- out：布局与需求一起交给建模；家具位置和空间连接同一版，不加第二份人工关系表
  - scripts/run.py：handoff保留布局及需求来源
- source → known：资料
- known → ask：否
- known → write：是
- ask → out：明确来源后

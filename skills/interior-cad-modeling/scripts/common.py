#!/usr/bin/env python3
"""Shared contract helpers for the full-floorplan CAD backend."""
...
SELECTION_SCHEMA = "interior.cad-asset-selection.v1"
CATALOG_SCHEMA = "interior.cad-residential-assets.v3"
...
def semantic_artifact_record(manifest: dict[str, Any], key: str) -> dict[str, Any]:
    candidates = [
        (candidate_key, value)
        for candidate_key, value in manifest.get("artifacts", {}).items()
...
def artifact_path(manifest_path: Path, manifest: dict[str, Any], key: str) -> Path:
    record = semantic_artifact_record(manifest, key)
...
def load_handoff(path: Path) -> tuple[dict[str, Any], dict[str, Any], dict[str, Any]]:
    path = path.resolve()
    handoff = read_json(path)
    if handoff.get("schema") != HANDOFF_SCHEMA:
        raise ValueError(f"handoff schema must be {HANDOFF_SCHEMA}")
    producer = handoff.get("producer", {})
    if producer.get("skill") != "interior-floorplan-planning" or not str(producer.get("version", "")).startswith("6."):
        raise ValueError("handoff producer must be a schema-compatible interior-floorplan-planning 6.x release")
    ...
    structure = read_json(artifact_path(path, handoff, "structureData"))
    traces = read_json(artifact_path(path, handoff, "traceComponents"))
    if structure.get("schema") != STRUCTURE_SCHEMA or traces.get("schema") != TRACE_SCHEMA:
        raise ValueError("handoff structure/trace schema mismatch")
    if len({handoff.get("floorplanId"), structure.get("floorplanId"), traces.get("floorplanId")}) != 1:
        raise ValueError("handoff floorplan IDs differ")
    return handoff, structure, traces


def source_objects(traces: dict[str, Any]) -> dict[str, dict[str, Any]]:
    rows: dict[str, dict[str, Any]] = {}
    for row in traces.get("objects", []):
        source_id = row.get("sourceObjectCandidateId")
        if not source_id or source_id in rows:
            raise ValueError("trace components require unique sourceObjectCandidateId")
        rows[source_id] = row
    if not rows:
        raise ValueError("trace components have no accepted objects")
    return rows


def coordinate_transform(structure: dict[str, Any]) -> dict[str, Any]:
    coordinate = structure["coordinateSystem"]
    width = float(coordinate["realWidthMeters"]) * 1000
    depth = float(coordinate["realDepthMeters"]) * 1000
    return {
        "units": "mm",
        "source": {"origin": "north-west", "units": "m", "axes": {"x": "east", "y": "south", "z": "up"}},
        "target": {"origin": "floorplan-center", "units": "mm", "axes": {"x": "east", "y": "north", "z": "up"}},
        "matrix4x4": [[1000, 0, 0, -width / 2], [0, -1000, 0, depth / 2], [0, 0, 1000, 0], [0, 0, 0, 1]],
        "inverseMatrix4x4": [[0.001, 0, 0, width / 2000], [0, -0.001, 0, depth / 2000], [0, 0, 0.001, 0], [0, 0, 0, 1]],
        "controlPoints": [
            {"source": [0, 0, 0], "target": [-width / 2, depth / 2, 0]},
            {"source": [width / 1000, 0, 0], "target": [width / 2, depth / 2, 0]},
            {"source": [width / 1000, depth / 1000, 0], "target": [width / 2, -depth / 2, 0]},
            {"source": [0, depth / 1000, 0], "target": [-width / 2, -depth / 2, 0]},
            {"source": [width / 2000, depth / 2000, 0], "target": [0, 0, 0]},
        ],
    }


def to_cad_xy(structure: dict[str, Any], point: list[float]) -> tuple[float, float]:
    coordinate = structure["coordinateSystem"]
    return (
        (float(point[0]) - float(coordinate["realWidthMeters"]) / 2) * 1000,
        (float(coordinate["realDepthMeters"]) / 2 - float(point[1])) * 1000,
    )

#!/usr/bin/env python3
"""Resolve one CAD component move with swept AABB first-contact stopping."""
...
def overlaps(a_min, a_max, b_min, b_max, epsilon=1e-3):
...
def colliders(bounds, delta, factor, obstacles):
    ...
    return [obstacle["id"] for obstacle in obstacles if overlaps(minimum, maximum, obstacle["bounds"]["min"], obstacle["bounds"]["max"])]
...
def main() -> int:
    ...
    result = {
        ...
        "acceptedDelta": [value * safe for value in delta],
